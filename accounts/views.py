...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #messages.success(request, 'Thank you for registering. we have sent you an verifivation email. Kindly verify.')
        ...
        if user is not None:
            try:
                ...
                if is_cart_item_exists:
                    ...
                    #getting variation of product by cart_id
                    product_variation = []
                    for item in cart_item:
                        variation = item.variations.all()
                        product_variation.append(list(variation))

                    #getting the cart item from the user to access the product variation
...
def forgotPassword(request):
    if request.method == "POST":
        ...
        if Account.objects.filter(email=email).exists():
            ...
            message = render_to_string('accounts/reset__password_email.html',{
                'user':user,
                'domain':current_site,
                'uid' : urlsafe_base64_encode(force_bytes(user.pk)),
                'token' : default_token_generator.make_token(user),
...
def resetPassword(request):
    if request.method == "POST":
        ...
        if password == confirm_password:
            ...
            messages.error(request, 'Password do not exist')
            return redirect('resetPassword')
    else:
